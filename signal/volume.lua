...
local function emit_volume_info()
    ...
    awful.spawn.easy_async_with_shell("pacmd list-sinks | awk '/\\* index: /{nr[NR+7];nr[NR+11]}; NR in nr'", function(stdout)
        ...
        if (volume_int ~= volume_old or muted_int ~= muted_old) and volume_int ~=nil then
...
local volume_script = [[
    bash -c "
    pactl subscribe 2> /dev/null | grep --line-buffered \"Ereignis »ändern« auf Ziel #\"
    "]]
...
local microphone_script = [[
  bash -c '
  pactl subscribe 2> /dev/null | grep --line-buffered "Ziel #"
  ']]
