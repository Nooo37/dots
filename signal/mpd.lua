...
awful.widget.watch(mpd_passed_script, update_interval, function(_, stdout)
  -- local n = require("naughty")
  -- n.notify({title=tostring(stdout), text="testets"})
  if stdout ~= old_percentage and stdout ~= nil then
    awesome.emit_signal("evil::mpd_passed", stdout)
    old_percentage = stdout
  end
end)
