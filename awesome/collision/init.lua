...
local function new(k)
  ...
  glib.idle_add(glib.PRIORITY_DEFAULT_IDLE, function()
      --[[
      I changed that part to my linking 
      Superkey (mod4) is always pressed when doing something with my wm
      Shift is pressed when dragging something
      Control is pressed when it has to do with tags
      I don't use a multimonitor setup so I don't have to handle with that
      --]]
    for k,v in pairs(keys) do
...
--                              { description = "Move tag screen to the "..key_name, group = "Collision" })
        if k == "left" or k =="right" then -- Conflict with my text editor, so I say no
          aw[#aw+1] = awful.key({ "Mod4",          "Control"         }, key_name, function () module.tag   (k,nil ,true) end,
                                { description = "Select tag to the "..key_name, group = "Collision" })
-- Should be dragging clients across tags not dragging tag position
--          aw[#aw+1] = awful.key({ "Mod4", "Shift", "Control"         }, key_name, function () module.tag   (k,true,true) end,
