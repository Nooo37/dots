;; The modline is a modifierd version of sumibis
;; For more information: https://github.com/Jimmysit0/sumibi
...
(with-eval-after-load 'calendar
  (defun calendar-setup-header ()
	...
	(face-remap-add-relative
	 'header-line `(:overline ,foreground
					:height 0.5
					:background ,background)))
...
(defun sumibi-modeline-status-icon (name &optional fmt fg-color bg-color)
  ...
  (let ((fg-color (or fg-color foreground))
		(bg-color (or bg-color red))
		(fmt      (or fmt "  %s  ")))
